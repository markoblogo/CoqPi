...
test('finder pipeline surface exposes score import draft and session labels', () => {
  ...
  assert.equal(ready.readinessLabel, 'ready for contact')
  assert.match(ready.readinessReason, /draft ready/)
  ...
  assert.equal(weakSurface.readinessLabel, 'enrich first')
  assert.match(weakSurface.readinessReason, /weak candidate/)
