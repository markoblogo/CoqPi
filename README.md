...
- Finder real source adapters v2 are stricter on messy real-world snippets: generic job-board text blocks without explicit field labels, accelerator snippets with `batch/cohort`, `who should apply`, and `equity terms`, plus investor aliases such as `HQ`, `sector focus`, `stage preference`, `initial check`, and `contact email`.
