...
- Real Finder Source Adapter v1 accepts owner-pasted URLs, vacancy/export text, or simple candidate lists for the selected job, extracts common fields (`company/partner`, `role/opportunity`, `country/city/location`, `contact`, `deadline`, `why relevant`, `missing info`), previews normalized candidates first, lets the owner select/edit them, and only then sends reviewed candidates into the same scoring/import pipeline.
...
1. Finder source adapter format pass: add dedicated parsers for common vacancy exports, LinkedIn-style snippets, accelerator pages, and investor lists while preserving local preview-first behavior.
