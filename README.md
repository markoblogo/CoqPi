...
- Real Finder Source Adapter v0 accepts owner-pasted URLs, vacancy/export text, or simple candidate lists for the selected job, previews normalized candidates first, lets the owner select/edit them, and only then sends reviewed candidates into the same scoring/import pipeline.
...
1. Finder source adapter enrichment: improve parsing for specific vacancy/candidate export formats and add richer field extraction before preview.
