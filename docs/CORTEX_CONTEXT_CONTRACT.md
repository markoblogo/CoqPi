...
- Knowledge extraction preview is metadata-only in this phase: title, adapter type, classification, missing fields, retrieval readiness, extraction mode, and provenance hash. It must not fetch URLs, scan folders, or expose raw file/link contents.
