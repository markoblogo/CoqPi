...
- Finder `owner_paste_v0` source adapter output is owner-provided evidence normalization only. URL strings are not fetched; pasted URLs, vacancy snippets, LinkedIn-style job snippets, accelerator/program snippets, investor/fund lists, partner exports, and CSV-like exports first become a no-write preview with deterministic parser-specific extraction for company/partner, role/opportunity, location, contact, deadline, relevance, and missing information, plus scenario-specific fields such as investor stage/ticket/thesis when present. Selected/owner-edited candidates are then appended as deterministic source-ID records and remain private/local until explicitly imported into session packs.
