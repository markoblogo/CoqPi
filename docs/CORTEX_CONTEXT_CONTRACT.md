...
- Knowledge pack assembly is owner-reviewed draft creation only. It may copy compact extracted fields into a local unselected pack form, but it must not persist a pack, select it for a session, or make it assistant-visible without a separate explicit save/selection action.
