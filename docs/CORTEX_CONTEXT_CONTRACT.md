...
This version creates a local shared-RAG ingress manifest plus bounded local extraction/pack assembly. It still does not create a broad retriever, vector store, source watcher, link crawler, provider call, or automatic promotion.
...
The current capture action is deliberately narrow. The owner must select `Capture & classify` for one `file` record. CoqPi then reads that file locally, calculates its SHA-256, and records a private append-only capture event. Direct readable text capture covers `.md`, `.txt`, `.csv`, and `.json`. Explicit document capture also allows `.pdf`, `.docx`, `.pptx`, `.xlsx`, `.xls`, and `.html` through a local `markitdown_v1` adapter. All of these remain capped at 10 MB and still end as compact local extraction fields rather than raw source exposure.
...
- Finder source adapter output remains private/local and owner-triggered. `owner_paste_v0` is owner-provided evidence normalization only: pasted URLs, vacancy snippets, LinkedIn-style job snippets, accelerator/program snippets, investor/fund lists, partner exports, and CSV-like exports first become a no-write preview with deterministic parser-specific extraction for company/partner, role/opportunity, location, contact, deadline, relevance, and missing information, plus scenario-specific fields such as investor stage/ticket/thesis when present. `public_page_v1` may fetch exactly one explicit public `http(s)` URL, derive a compact local title/description/heading/excerpt preview, and then route that preview through the same reviewed candidate flow. For thin deterministic previews only, CoqPi may optionally run one local Crawl4AI markdown enrichment pass and keep it only when it improves the compact candidate quality; absence or failure of that local adapter must not fail the preview request. If both automatic paths remain weak, `manual_complex_page_v1` allows a supervised escape hatch for the same public URL using owner-reviewed notes/markdown pasted manually; it stays local and does not automate a browser, authenticate, or emit outbound actions. Each candidate is labeled with one bounded parser pack from `job_page_v1 | investor_fund_v1 | accelerator_program_v1 | company_profile_v1`; this label is compact metadata only and does not authorize broader retrieval or raw-content exposure. No scheduler, batch crawl, browser automation, auth session, search API, or outbound action is allowed in either mode. Selected/owner-edited candidates are then appended as deterministic source-ID records and remain private/local until explicitly imported into session packs.
