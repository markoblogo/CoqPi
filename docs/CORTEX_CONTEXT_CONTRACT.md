...
- Knowledge pack assembly is owner-reviewed draft creation only. It may copy compact extracted fields into a local unselected pack form and show a review surface with summary, context, links, weak fields, retrieval state, local lifecycle events, and filters for status, selected/unselected visibility, assistant-ready entries, weak fields, and stale events. `assembled`, `reviewed`, and `saved` events may be written to local manifest history, but raw source paths are redacted from lifecycle reasons and no pack becomes assistant-visible without a separate explicit save/selection action.
