...
- Finder `owner_paste_v0` source adapter output is owner-provided evidence normalization only. URL strings are not fetched; pasted exports first become a no-write preview, then selected/owner-edited candidates are appended as deterministic source-ID records and remain private/local until explicitly imported into session packs.
