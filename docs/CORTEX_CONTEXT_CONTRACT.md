...
- Finder `owner_paste_v0` source adapter output is owner-provided evidence normalization only. URL strings are not fetched; pasted exports are compacted into candidate records with deterministic source IDs and remain private/local until explicitly imported into session packs.
