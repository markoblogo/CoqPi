...
- **Finder -> session handoff**: queue decisions and outreach-draft status changes can immediately alter `selectedCounterpartyPackIds` and `selectedFinderOutreachDraftId`, so Prepare, Live, and the next assistant request stay aligned without reload/save workarounds.
