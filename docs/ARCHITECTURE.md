...
- **Knowledge pack assembly/review/lifecycle**: reviewed extraction fields can populate an unselected local context pack draft. A review surface shows summary, context, links, weak fields, retrieval state, and local lifecycle events. `assembled`, `reviewed`, and `saved` entries are appended to the Personal Knowledge Core manifest without making raw source content assistant-visible.
