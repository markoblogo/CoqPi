...
- **Finder source adapter v1**: `owner_paste_v0` normalizes owner-pasted URLs, vacancy/export text, LinkedIn-style job snippets, accelerator/program snippets, investor/fund lists, partner exports, and CSV-like candidate exports for a selected job into a no-write preview. It now routes entries through deterministic parsers (`url`, `structured_fields`, `linkedin_job`, `accelerator_snippet`, `investor_list`, `partner_export`) and enriches compact local fields such as company/partner, role/opportunity, location, contact, deadline, relevance, stage/ticket/thesis where available, and missing information. The owner can select and edit candidates before the reviewed draft is appended. URLs are stored as provenance links only; CoqPi does not fetch them in this phase.
