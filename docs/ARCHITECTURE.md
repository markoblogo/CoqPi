...
- **Knowledge pack assembly/review/lifecycle**: reviewed extraction fields can populate an unselected local context pack draft. A review surface shows summary, context, links, weak fields, retrieval state, local lifecycle events, and filters for status, selected/unselected visibility, assistant-ready entries, weak fields, and stale events. `assembled`, `reviewed`, and `saved` entries are appended to the Personal Knowledge Core manifest without making raw source content assistant-visible.
