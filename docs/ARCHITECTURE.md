...
- **Finder source adapter v0**: `owner_paste_v0` normalizes owner-pasted URLs, vacancy/export text, or simple candidate lists for a selected job into a no-write preview. The owner can select and edit candidates before the reviewed draft is appended. URLs are stored as provenance links only; CoqPi does not fetch them in this phase.
