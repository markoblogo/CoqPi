...
- **Finder candidate scoring**: preview candidates get deterministic scenario-aware `fitScore`, `missingInfo`, and `nextAction` for `job`, `partner`, `investor`, `accelerator`, and `other`. Owner-provided review fields override generated defaults.
