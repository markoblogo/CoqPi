...
- **Knowledge ingestion quality**: shared readiness helpers summarize pending/hash-only/retrieval-ready sources, pointer-only boundaries, retention expiry, selected pack quality, and whether the current strict candidate set is clean enough for a future vector adapter.
